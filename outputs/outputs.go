...
func humanizeResult2(r resource.TestResult) string {
	...
	case resource.SUCCESS:
		...
		case resource.Values:
			return green("%s: %s: %s: all expectations found: [%s] raw: [%s]", r.ResourceType, r.ResourceId, r.Property, strings.Join(r.Expected, "\n"), strings.Join(r.Found, "\n"))
	...
	case resource.FAIL:
		...
		case resource.Values:
			return red("%s: %s: %s: expectations not found: expect: [%s] found: [%s]", r.ResourceType, r.ResourceId, r.Property, strings.Join(r.Expected, "\n"), strings.Join(r.Found, "\n"))
		case resource.Contains:
			return red("%s: %s: %s: patterns not found: expect: [%s] found: [%s]", r.ResourceType, r.ResourceId, r.Property, strings.Join(r.Expected, "\n"), strings.Join(r.Found, "\n"))
		default:
			return red("Unexpected type %d", r.TestType)
		}
	case resource.SKIP:
